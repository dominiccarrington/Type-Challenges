...
type PropsDescription<T> = Constructor<T> | {type?: Constructor<T>|Constructor<T>[]};

type GetComputed<A extends {[key: string]: () => any}> = {
  [K in keyof A]: ReturnType<A[K]>
};

type GetProps<Props> = {
  [K in keyof Props]: Props[K] extends PropsDescription<infer PropType>
   ? PropType extends String ? string
   : PropType extends Number ? number
   : PropType extends BigInteger ? bigint
   : PropType extends Boolean ? boolean
   : PropType extends object ? PropType
   : any
   : never;
};

declare function VueBasicProps<
  Props extends {[key: string]: PropsDescription<any>},
  Data extends {[key: string]: any},
  Computed extends {[key: string]: (this: Data) => any},
...
>(options: {
  ...
  computed: Computed,
  methods: Methods
} & ThisType<Data & GetComputed<Computed> & Methods & GetProps<Props>>): any
